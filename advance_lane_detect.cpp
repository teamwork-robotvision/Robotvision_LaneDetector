...
cv::Mat LaneDetector::calHist(cv::Mat inputImage) {
    cv::Mat histogram(1, inputImage.cols, CV_64F);//定义一个一行x列的矩阵
    for(int k=0;k<inputImage.cols;k++)
        histogram.at<double>(0,k)=0;
//    Mat_<Vec3b>::iterator diedaiqi = image.begin<Vec3b>();
for(int i=0;i<inputImage.rows;i++){
    for(int j=0;j<inputImage.cols;j++){
        histogram.at<double>(0,j)=inputImage.at<double>(i,j);
    }
}
return histogram;
}
...
//    cout<<leftx_base;
    ...
    vector<cv::Point> left_lane_inds;
    vector<cv::Point> right_lane_inds;
    ...
    for(int window=0;window<nwindows;window++){
        ...
        //        左车道
        vector<cv::Point>  good_left_inds;

        for(vector<cv::Point>::iterator non_indL=nonzero.begin();non_indL!=nonzero.end();non_indL++){

            if(((*non_indL).y>=win_y_low)&((*non_indL).y< win_y_high)&
               ((*non_indL).x>= win_xleft_low)&((*non_indL).x<win_xleft_high)){

                if((*non_indL).x!=0&&(*non_indL).y!=0)
                good_left_inds.push_back((*non_indL));//将找到的车道的最佳点添加进去---放进去的元素都是点

            }

        }
        //vector<int>::iterator iter=ivec.begin();iter!=ivec.end();++iter
        //        右车道
        vector<cv::Point>   good_right_inds;
        for(vector<cv::Point>::iterator non_indR=nonzero.begin();non_indR!=nonzero.end();non_indR++){

            if(((*non_indR).y>=win_y_low)&((*non_indR).y< win_y_high)&
               ((*non_indR).x>= win_xright_low)&((*non_indR).x<win_xright_high)){
                if((*non_indR).x!=0&&(*non_indR).y!=0)
                good_right_inds.push_back((*non_indR));       //将找到的车道的最佳点添加进去---放进去的元素都是点cv::Point类型
            }

        }
        //        # Append these indices to the lists

        for( vector<cv::Point>::iterator Left=good_left_inds.begin();Left!=good_left_inds.end();Left++){

            left_lane_inds.push_back((*Left));
        }
        for( vector<cv::Point>::iterator Right=good_right_inds.begin();Right!=good_right_inds.end();Right++){
//            if((*Right).x!=0&&(*Right).y!=0)

            left_lane_inds.push_back((*Right));
        }
        ...
        if(good_left_inds.size()>minpix){
            unsigned int sumL=0,meanL;

            for(vector<cv::Point>::iterator i=good_left_inds.begin();i!=good_left_inds.end();i++){
                sumL=sumL+(*i).x;
            }
        ...
        if(good_right_inds.size()>minpix){
            unsigned int sumR=0,meanR;

            for(vector<cv::Point>::iterator i=good_right_inds.begin();i!=good_right_inds.end();i++){

                sumR=sumR+(*i).x;
            }
            meanR=sumR/good_right_inds.size();
            rightx_current=meanR;
        }
...
//    for(vector<cv::Point>::iterator left=left_lane_inds.begin();left!=left_lane_inds.end();left++){

//        left_line.push_back((*left));
//    }
//    vector<cv::Point> right_line;
//    //对left_lane_inds和right_lane_inds中的点进行拟合
//    for(vector<cv::Point>::iterator right=right_lane_inds.begin();right!=left_lane_inds.end();right++){
...
//    cout<<left_lane_inds.size()<<endl<<right_lane_inds.size()<<endl;
////    cout<<left_lane_inds<<endl;

    //cvWaitKey(0);
//    getchar();
    left_lane=polyfit(left_lane_inds,9);
    right_lane=polyfit(right_lane_inds,9);
